...
| work (job hub) | `work.prisma` (WorkWorkspace, WorkArea, WorkProject, WorkPerson, WorkMeeting, WorkTask, WorkNote, WorkReview) | `work/` (split: `shared` ⚠️ raw SQL + runtime `ensureWorkTables()`, `reads`, `mutations`, barrel `index.ts`) | `work/` (single page, `?tab=` sections: today/projects/areas/people/meetings/notes/review; forms in `_components/`) | `work.tsx` (today's meetings + must-do tasks) | `work.ts` (day/week keys, meeting recurrence-on-day, today task grouping, due labels) |
